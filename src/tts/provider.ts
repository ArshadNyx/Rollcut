...
/** One synthesized narration line, already decoded to a uniform wav. */
...
export interface TtsProvider {
  ...
  /** Synthesize `text` to a wav on disk and report its exact duration. */
  synthesize(text: string, voice: string | undefined, outPath: string): Promise<Clip>;
}
...
export async function toWav(input: string, outPath: string): Promise<string> {
  // prettier-ignore
  await ffmpeg([
    '-i', input,
...
/** A narration line tied to the step it belongs to. */
export interface Narration extends Clip {
  stepIndex: number;
  text: string;
}
...
export async function planNarration(
  notes: { stepIndex: number; text: string }[],
  voice: string | undefined,
  dir: string,
  provider: TtsProvider,
  onClip?: (n: Narration) => void,
): Promise<Narration[]> {
  ...
  for (const { stepIndex, text } of notes) {
    const clip = await provider.synthesize(
      text,
      voice,
      join(dir, `note-${String(stepIndex).padStart(2, '0')}.wav`),
    );
    const narration: Narration = { ...clip, stepIndex, text };
    out.push(narration);
    onClip?.(narration);
  }
  return out;
