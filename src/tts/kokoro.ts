...
const DTYPE = (process.env.ROLLCUT_KOKORO_DTYPE || 'q8') as 'fp32' | 'fp16' | 'q8' | 'q4' | 'q4f16';
...
async function load(onProgress?: (message: string) => void): Promise<KokoroModel> {
  if (!modelPromise) {
    modelPromise = (async () => {
      ...
      try {
        ...
        throw new Error(
          'The kokoro TTS provider needs `kokoro-js`, which is an optional dependency and is not installed here.\n' +
            '  Install it:        pnpm add kokoro-js\n' +
            '  Or use Edge:       --tts edge\n' +
            '  Or skip narration: --no-narration',
          { cause: e },
        );
      }
...
export const kokoroTts: TtsProvider = {
  ...
  async synthesize(text, voice, outPath): Promise<Clip> {
    ...
    if (!(selected in model.voices)) {
      ...
      throw new Error(
        `Unknown kokoro voice \`${selected}\`. Kokoro uses its own names (not Edge's) — try \`${DEFAULT_VOICE}\`. ` +
