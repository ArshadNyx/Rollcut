/** A click to zoom toward, in video time and source pixels. */
...
export const ZOOM_TIMING: ZoomTiming = { inMs: 620, holdMs: 520, outMs: 820 };
 ...
 */
function envelope(event: ZoomEvent, timing: ZoomTiming): string {
  ...
  const rampOut = smoothstep(
    `1-(in_time-${holdEnd.toFixed(3)})/${(timing.outMs / 1000).toFixed(3)}`,
  );
...
export interface ZoomFilterOptions {
  ...
  scale?: number;
  timing?: ZoomTiming;
  /** Upscale before zooming, to soften zoompan's whole-pixel rounding. */
  supersample?: number;
}
