...
export async function ffmpeg(args: string[]): Promise<void> {
  if (!BIN) {
    throw new Error(
      'ffmpeg-static did not provide a binary for this platform — run `pnpm install` again, or set ROLLCUT_FFMPEG to an ffmpeg on your PATH.',
    );
  }
  const bin = process.env.ROLLCUT_FFMPEG || BIN;
  const argv = ['-hide_banner', '-nostdin', '-y', ...args];
  if (debug) console.error(`[ffmpeg] ${bin} ${argv.join(' ')}`);
