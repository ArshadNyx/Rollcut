/** A subtitle line with absolute timings against the finished video. */
...
export interface SubtitleStyle {
  /**
   * Arial is the one safe choice across build machines: it exists on macOS and
   * Windows, and fontconfig aliases it to the metric-compatible Liberation Sans
   * on Linux runners. Naming a font that is missing makes libass substitute a
   * face with different metrics, so the same spec would wrap differently in CI
   * than it does locally.
   */
  fontName: string;
  /** Video size; ASS sizes are in these units, so this makes them real pixels. */
  width: number;
  height: number;
  fontSize: number;
  /** Distance from the frame edge to the text box, in pixels. */
  marginH: number;
  marginV: number;
}
 ...
 */
export function maxChars(style: SubtitleStyle = DEFAULT_STYLE): number {
...
function srtTime(ms: number): string {
  ...
  return `${pad(Math.floor(t / 3_600_000))}:${pad(Math.floor((t % 3_600_000) / 60_000))}:${pad(
...
function assTime(ms: number): string {
  ...
  return `${Math.floor(t / 3_600_000)}:${pad(Math.floor((t % 3_600_000) / 60_000))}:${pad(
    Math.floor((t % 60_000) / 1000),
  )}.${pad(cs)}`;
}
 ...
 */
export function wrap(text: string, limit = maxChars()): string[] {
 ...
 */
export function toAss(cues: Cue[], style: SubtitleStyle = DEFAULT_STYLE): string {
  const header = [
    '[Script Info]',
    'ScriptType: v4.00+',
  ...
  const events = usable(cues).map(
    (cue) =>
      `Dialogue: 0,${assTime(cue.startMs)},${assTime(cue.endMs)},Rollcut,,0,0,0,,` +
