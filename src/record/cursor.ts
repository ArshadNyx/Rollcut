 ...
 */
function overlayScript(scale: number, zoomMs: number): string {
  return `
(() => {
 ...
 */
export async function setZoomEnabled(page: Page, enabled: boolean): Promise<void> {
  await page
    .evaluate((off: boolean) => {
      (window as unknown as { __rollcutNoZoom: boolean }).__rollcutNoZoom = off;
    }, !enabled)
    .catch(() => undefined);
}
