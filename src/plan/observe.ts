 ...
 */
function collectorScript(maxElements: number, maxHeadings: number): string {
  return `(() => {
...
function navigationCandidates(landing: PageObservation, origin: string): string[] {
  ...
  try {
    ...
    for (const candidate of navigationCandidates(landing, origin)) {
      ...
      try {
        ...
        if (pages.some((seenPage) => seenPage.path === observed.path)) continue;
        pages.push(observed);
