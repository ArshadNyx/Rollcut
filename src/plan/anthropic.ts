 ...
 */
export const anthropicPlanner: PlanProvider = {
  name: 'anthropic',
  ...
  async propose(request: ProposeRequest): Promise<unknown> {
    ...
    try {
      ...
      throw new Error(
        'The planner needs `@anthropic-ai/sdk`, which is an optional dependency and is not installed here.\n' +
    ...
    const response = await client.messages
      ...
      .catch((e: unknown) => {
        ...
        if (/authentication|api[_ -]?key|401/i.test(message)) {
          throw new Error(
            'No Anthropic credentials found. Set one and try again:\n' +
    ...
    if (response.stop_reason === 'refusal') {
      throw new Error(
        `The model declined to plan this page${
    ...
    if (!text.trim()) {
      throw new Error(
        `The model returned no plan (stop_reason: ${response.stop_reason}). Try again, or narrow the page.`,
      );
    }
