  ...
  for (const action of capture.actions) {
    ...
    switch (action.kind) {
      case 'navigate':
        ...
        break;
      case 'click':
        if (action.selector) produced.push({ click: action.selector });
        break;
      case 'type':
        if (action.text) {
        ...
        break;
      case 'press':
        if (action.key) produced.push({ press: action.key });
        break;
      case 'scroll':
        if (action.pixels) produced.push({ scroll: Math.round(action.pixels) });
        break;
    }
    ...
    if (action.kind === 'navigate' && action.text) titles.set(steps.length, action.text);
    steps.push(...produced);
  }
 ...
 * actually made.
 */
function dropRepeatedClicks(steps: Record<string, unknown>[]): Record<string, unknown>[] {
  const out: Record<string, unknown>[] = [];
  for (const step of steps) {
    const previous = out[out.length - 1];
    const repeat =
      'click' in step && previous && 'click' in previous && previous.click === step.click;
    if (repeat) continue;
...
export interface NarrateOptions {
  ...
  /** Extra context, e.g. a README, so the notes know what the product is. */
  readme?: string;
...
export async function narrate(options: NarrateOptions): Promise<NarrateResult> {
  ...
  if (options.provider) {
    ...
    })) as { notes?: { step: number; note: string | null }[] };
  ...
  if (!parsed.success) {
    throw new Error(
      `The captured run did not produce a valid spec: ${parsed.error.issues
