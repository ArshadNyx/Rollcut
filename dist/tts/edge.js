...
async function ensureAvailable() {
    ...
    try {
        ...
        throw new Error(`edge-tts is not available via \`${PYTHON} -m edge_tts\`. Install it with:\n` +
...
export const edgeTts = {
    ...
    async synthesize(text, voice, outPath) {
        ...
        try {
            ...
            throw new Error(`edge-tts failed on "${text.slice(0, 60)}${text.length > 60 ? '…' : ''}" with voice \`${voice || DEFAULT_VOICE}\`. ` +
                `Check the voice name against \`${PYTHON} -m edge_tts --list-voices\` and that you are online. ${detail}`, { cause: e });
        }
