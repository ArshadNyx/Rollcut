...
export function presetPlanner(name, preset) {
    return {
        name,
        async propose(request) {
            const endpoint = (preset.urlEnv ? process.env[preset.urlEnv] : undefined) || preset.endpoint;
            if (!endpoint) {
                throw new Error(`\`--llm ${name}\` needs an endpoint. Set ${preset.urlEnv ?? 'the endpoint'} to an OpenAI-compatible chat completions URL.`);
            }
